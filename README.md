...
- Agent 长任务不再被固定 20 次工具调用上限截断，更适合持续读写文件、分批分析和多步骤执行。
- 对连续重复同一批工具调用新增保护，模型卡在同一动作时会停下并给出明确说明，避免无意义循环。
- 发布门禁继续要求真实手机前端聊天路径通过，而不是只跑模型 ping 或构建。
