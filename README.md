...
- 工具型任务在发送前给出能力预警，发送后在聊天流留下系统证据提示，避免把没有工具执行证据的回复误判为已完成。
- 回看历史时明确要求以工具执行卡、文件变更卡或“工具受限，未真实执行”为准，减少模型空口承诺。
- 真机门禁覆盖轻量模型连通、正式 `stream=true` 聊天、工具/Agent 路径和聊天 UI 证据提示。
- App 版本同步到 `1.1.30 / versionCode 140`，方便真机与 Release 对齐。
