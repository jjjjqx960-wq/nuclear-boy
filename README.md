...
- 正式聊天失败升级为结构化诊断卡：识别 `model_not_found`、上游 provider 缺少凭证、空回复和请求格式错误。
- 路由失败卡会直接提示“获取模型列表、点选真实返回模型名”或“补齐 provider 上游 Key/额度”，避免只看到 404。
- Debug 会话种子广播支持 base64 文案，真机 UI 门禁可稳定注入带冒号、斜杠和换行的失败消息。
- App 版本同步到 `1.1.34 / versionCode 144`，方便真机与 Release 对齐。
