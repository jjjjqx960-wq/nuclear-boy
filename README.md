...
- 工具型请求完成后新增后置结果复核：没有工具执行卡、文件变更卡，也没有明确“工具受限，未真实执行”时，会自动提示不要当作已完成。
- 发送前预警、模型侧本轮约束、发送后证据提示和后置复核形成闭环，减少模型口头伪造读写/运行结果。
- 真机门禁继续覆盖轻量模型连通、正式 `stream=true` 聊天、工具/Agent 路径和聊天 UI 证据提示。
- App 版本同步到 `1.1.32 / versionCode 142`，方便真机与 Release 对齐。
