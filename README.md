...
- 第三方模型“测试聊天”升级为两阶段：先轻量 ping，再用同一配置验证正式 stream=true 聊天链路。
- 正式测试会携带诊断工具定义，能提前暴露“ping 成功但聊天页/Agent 路径失败”的网关兼容问题。
- 测试排障包同时复制轻量请求和正式 stream 请求模板，继续保持 Key 脱敏。
