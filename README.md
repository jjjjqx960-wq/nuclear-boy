...
- 工具型请求会向模型注入仅本轮生效的真实性约束：没有真实工具结果时必须明说“工具受限，未真实执行”。
- 用户可见聊天流仍保留工具能力证据提示，模型侧和界面侧同时防止“假装已读取/写入/运行”。
- 真机门禁继续覆盖轻量模型连通、正式 `stream=true` 聊天、工具/Agent 路径和聊天 UI 证据提示。
- App 版本同步到 `1.1.31 / versionCode 141`，方便真机与 Release 对齐。
