...
`app.diagnostics` 属于整体层诊断编排目录，负责把 API、模型配置、Python 执行器、文件工具和工具注册表组合成可运行的自检流程。

## 边界

这里只做跨模块诊断编排和 ADB 调试入口，不实现底层模型协议、文件系统细节或 Python 运行时。调试写配置入口只能作为 debug 包的运维辅助能力。
...
验证设置页全量诊断、ADB 广播诊断、debug 模型配置写入、模型连通性、Python 执行、文件写读删和工具注册表执行结果。
