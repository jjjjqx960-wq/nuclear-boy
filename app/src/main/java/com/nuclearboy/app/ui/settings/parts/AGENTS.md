...
`settings/parts` 是设置页零件层，放置最小可复用的展示文案、状态判断、Key 脱敏指纹、模型名清理提示、端点预览、完整地址风险提示、模型测试/全量自检复制摘要和格式化规则。
...
新增规则必须补单元测试，覆盖关键分支、脱敏输出、输入期和测试期模型名清理提示、端点预览、完整地址风险提示、模型测试/全量自检复制摘要和兜底文案。
