...
2026-06-15 1.1.57 ToolDraftHintUiTest 改用“帮我把 API key 轮换一下”真实输入路径，验证凭证轮换类草稿仍触发远程配置工具能力预警。
2026-06-15 1.1.56 ToolDraftHintUiTest 改用“把这个模型从网关删掉”真实输入路径，验证远程配置删除/下线类草稿仍触发远程配置工具能力预警。
2026-06-15 1.1.55 ToolDraftHintUiTest 改用“把聊天模型切到 deepseek”真实输入路径，验证当前聊天模型切换类草稿仍触发远程配置工具能力预警。
2026-06-15 1.1.54 ToolDraftHintUiTest 改用“把默认模型设为 deepseek”真实输入路径，验证默认模型设置类草稿仍触发远程配置工具能力预警。
2026-06-15 1.1.53 ToolDraftHintUiTest 改用“把网关模型换成 deepseek”真实输入路径，验证网关模型切换类草稿仍触发远程配置工具能力预警。
2026-06-15 1.1.52 ToolDraftHintUiTest 改用“帮我把这个 key 填到后台配置里”真实输入路径，验证后台配置填入类草稿仍触发远程配置工具能力预警。
2026-06-15 1.1.51 ToolDraftHintUiTest 改用“帮我把这个模型加入网关”真实输入路径，验证加入/接上/放到/挂到网关类表达仍触发远程配置工具能力预警。
2026-06-15 1.1.50 ToolDraftHintUiTest 将 API 学习免打扰样本升级为“请调用接口时怎么带参数”，覆盖含执行字样但本质为参数咨询的真实前端输入。
2026-06-15 1.1.49 ToolDraftHintUiTest 新增学习型 API 草稿不显示工具能力预警的真实前端断言，防止普通咨询被打断。
