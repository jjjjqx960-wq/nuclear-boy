## 目录职责

`ui-chat/src/test` 承载 `ui-chat` 模块的 JVM 单元测试，用于验证零件层纯函数和轻量组件外的可测试规则。

## 边界

这里只放测试代码和测试辅助数据，不放生产实现。涉及 Compose 渲染或 Android 系统交互的测试应另建合适的 Android 测试目录。

## 允许依赖

允许依赖 `ui-chat` 主源码、`common` 测试可见模型、JUnit 和 Kotlin 标准库。

## 禁止事项

不要在测试中读取真实用户文件、账号、Token、签名密钥或个人数据。不要依赖本机绝对路径。

## 常用命令

- `./gradlew :ui-chat:testDebugUnitTest`
- `./gradlew :ui-chat:compileDebugKotlin`

## 验证方式

优先验证纯函数输入输出、正式聊天失败诊断识别、工具受限提示识别、工具型草稿预警识别、防假执行约束追加和去重、发送后工具能力证据提示、工具型请求后置结果复核、后置复核结构化提示识别、本轮模型真实性约束、普通聊天不误判、普通错误不误判、摘要胶囊语义描述、边界排序、排序模式语义描述、选择切换、提示格式、全选/补选数量标签、只看/全部切换数量标签、当前可见引用标签、过滤摘要范围标签、过滤清除语义描述、空结果文案、清除过滤动作显示条件、无已选可见总大小文案、批量引用大小反馈、单文件引用相对路径反馈、单文件引用大小反馈、图标操作数量语义、引用数量标签和空集合行为，再由模块编译验证 UI 接入。
