...
`parts` 是 `ui-chat` 的零件层，承载聊天 UI 可复用的最小数据结构、常量和纯函数，例如命令元数据、滚动状态判断、文件面板过滤规则、文件面板摘要计算、文件面板排序规则和文件引用提示生成。
...
优先验证纯函数输入输出，包括空输入、大小写、扩展名、路径匹配、目录/文件计数、类型分布、排序优先级、选择切换和引用提示格式，再由组件层和部件层编译验证接入效果。
