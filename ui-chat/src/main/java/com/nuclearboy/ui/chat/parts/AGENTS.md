...
`parts` 是 `ui-chat` 的零件层，承载聊天 UI 可复用的最小数据结构、常量和纯函数，例如命令元数据、滚动状态判断、文件面板过滤规则、文件面板摘要计算和文件面板排序规则。
...
优先验证纯函数输入输出，包括空输入、大小写、扩展名、路径匹配、目录/文件计数、类型分布和排序优先级，再由组件层和部件层编译验证接入效果。
