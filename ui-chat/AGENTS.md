...
`ui-chat` 是部件层模块，负责聊天界面、聊天 ViewModel、斜杠命令、消息渲染、Token HUD 和聊天侧工作区交互，包括文件浏览、过滤、摘要、排序、预览和引用入口。
...
验证普通发送、`/goal`、`/loop`、`/stop`、`/compact`、`/rewind`、消息持久化、流式状态更新，以及文件面板浏览、过滤、摘要、排序、预览和引用链路。
