...
`api-deepseek` 源码包承载模型服务协议、第三方网关配置、endpoint 构造、模型名和服务地址清理、诊断提示、SSE 解析与 Token 统计。
...
修改协议、endpoint、模型名或服务地址清理逻辑后，必须运行 `:api-deepseek:test`，并在涉及 app 接入时补跑 `:app:testDebugUnitTest` 和 `:app:assembleDebug`。
