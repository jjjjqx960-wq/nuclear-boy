...
检查 DTO 序列化、SSE 事件解析、Token 统计、第三方 OpenAI 兼容网关请求体、第三方工具格式兼容降级、第三方工具请求连续 5xx/Retry-After 后兼容降级、兼容降级时工具结果保留为普通上下文、404 模型路由诊断提示、/v1/models 探测、公开模型名/服务地址隐藏字符清理函数和上游凭证短重试。
