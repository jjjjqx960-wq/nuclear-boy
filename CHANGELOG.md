2026-06-15 1.1.52 API/远程配置草稿预警补齐后台配置填入和部署上线口语：`帮我把这个 key 填到后台配置里`、接到/填到/填进/填上/部署到/发布到/上线到/挂进等表达会识别为需要真实接口或远程配置能力，同时 `怎么把 key 填到后台配置里` 仍按咨询免打扰。
2026-06-15 1.1.51 API/远程配置草稿预警补齐执行口语：`帮我把这个模型加入网关`、加入/接上/放到/放进/挂到/挂上等表达会识别为需要真实接口或远程配置能力，避免用户没有说“API”或“加一下”时漏掉证据链提示。
2026-06-15 1.1.50 API 学习/咨询免打扰继续细化：`请调用接口时怎么带参数`、请求体/header/鉴权/字段/返回值等用法咨询不再因为包含“请调用接口”而误触发工具能力预警，保留真实执行型 API/远程配置草稿的证据链要求。
2026-06-15 1.1.49 API 草稿预警增加学习/咨询型问题免打扰：`怎么调用接口添加模型`、`如何用 API` 等只是在问用法的输入不再显示“可能需要工具能力”，但带“走 API 给我加进去”等真实执行意图的表达仍会预警，减少普通问答被误判成远程操作任务。
