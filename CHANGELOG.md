...
2026-06-15 1.1.59 API/远程配置草稿预警补齐账号安全和权限操作口语：`把这个用户封禁`、`帮我给这个账号开管理员权限`、`把登录密码重置一下` 等表达会识别为需要真实接口或远程配置能力，同时 `怎么封禁用户` 和 `权限说明文档` 不误报。
2026-06-15 1.1.58 API/远程配置草稿预警补齐账号计费操作口语：`帮我给这个账号充值`、`把账户额度补一下`、`帮我把这笔账单开票` 等表达会识别为需要真实接口或远程配置能力，同时 `怎么给账号充值`、`帮我设计一个账号体系` 和 `账号模型类` 不误报。
2026-06-15 1.1.57 API/远程配置草稿预警补齐凭证轮换/撤销口语：`帮我把 API key 轮换一下`、`把这组密钥撤销掉` 等表达会识别为需要真实接口或远程配置能力，同时 `如何轮换 API key` 和 `key-value 配置模型类` 不误报。
2026-06-15 1.1.56 API/远程配置草稿预警补齐删除/下线/禁用口语：`把这个模型从网关删掉`、`帮我把 provider 停用` 等表达会识别为需要真实接口或远程配置能力，同时 `怎么把模型从网关删掉` 和普通“服务下线通知”不误报。
2026-06-15 1.1.55 API/远程配置草稿预警补齐当前聊天模型切换口语：`把聊天模型切到 deepseek`、`帮我把当前模型改用 deepseek` 等表达会识别为需要真实接口或远程配置能力，同时不放宽到裸“模型”，避免 `帮我创建一个用户模型类` 误报。
2026-06-15 1.1.54 API/远程配置草稿预警补齐默认模型设置口语：`把默认模型设为 deepseek`、设为/设成/设置为/设置成/设默认/设成默认/设为默认等表达会识别为需要真实接口或远程配置能力，同时 `怎么把默认模型设为 deepseek` 仍按咨询免打扰。
2026-06-15 1.1.53 API/远程配置草稿预警补齐网关模型切换口语：`把网关模型换成 deepseek`、改成/改为/换成/换为/替换成/替换为等表达会识别为需要真实接口或远程配置能力，同时 `怎么把网关模型换成 deepseek` 仍按咨询免打扰。
